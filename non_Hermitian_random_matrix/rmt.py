# This file is mainly from Kwant
...
def circular(n, sym='A', charge=None, rng=None):
    ...
    if sym in ('D', 'DIII') and charge is not None:
        ...
        if sym == 'DIII':
            det *= (-1) ** (n // 2)
        if (charge > 0) != (det > 0): # if the sign of det is not same as the charge
            idx = np.arange(n)
            idx[-1] -= 1
            idx[-2] += 1
            s = s[idx]
        ...
        #print("Hermitian s: ", np.allclose(s, np.transpose(np.conjugate(s))))
        #print(s)
